...
class Request
{
     ...
     * @var array
     */
    private const ACTIONS_NEED_DUMMY_PARAM = [
        'deleteWebhook',
        'getWebhookInfo',
        'getMe',
    ];
     ...
     * @var string
     */
    private const API_BASE_URI = 'https://api.telegram.org';
    ...
    {
        if (empty($this->_data['download_path'])) {
            throw new TelegramException('Download path not set!');
        }

        $tg_file_path = $file->getFilePath();
        $file_path = $this->_data['download_path'] . '/' . $tg_file_path;
        ...
        try {
            $this->_client->get(
                '/file/bot' . $this->_data['api_key'] . '/' . $tg_file_path,
    ...
    public function send ($action, array $data = [])
    {
        ...
        $bot_username = $this->_data['bot_username'];
     ...
     * @throws \Longman\TelegramBot\Exception\TelegramException
     */
    public function addWebhook ($url, array $data = [])
    {
